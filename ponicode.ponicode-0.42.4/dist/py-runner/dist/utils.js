...
const createTmpFile = (sourceFilePath, code, log) => {
    ...
    const fileName = `__poni_test_${uuid_1.v4()}_${path.basename(sourceFilePath)}`;
    try {
        fs_1.writeFileSync(path.resolve(filePath, fileName), code, { flag: "w+" });
    }
    catch (err) {
...
const removeTmpFile = (filePath, fileName, log) => {
    try {
        fs_1.unlinkSync(path.resolve(filePath, fileName));
    }
    catch (err) {
